...
- 表管理（菜单 3.5）：按后端只读诊断（表是否存在、input/output/forward 策略、规则数量），并检测各后端之间的策略差异。
- 首次使用时初始化 nftables 表，绝不覆盖已配置的链。
- 将某个后端的链策略重置为 ACCEPT，修复旧策略遗留下来的孤立限制性策略。
- 清空指定后端，或一次性同步清空所有检测到的后端。
- 完全删除 nftables 表（高级操作，超出仅清空规则的范围）。
