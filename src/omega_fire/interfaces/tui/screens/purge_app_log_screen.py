...
"""Sous-ecran de gestion/purge du journal applicatif (bouton 'Gerer/
Purger' de l'ecran 1.5). Memes 4 modes qu'action_1_5_app_log::_do_manage_log
(30 jours / 120 jours / date precise / N entrees les plus anciennes) —
memes methodes AppLogger.clear()/delete_oldest() que AuditLogger, donc
copie quasi verbatim de purge_audit_history_screen.py (7.3), en ciblant
app_logger au lieu de audit_port."""
...
_ACTION_TITLE = "1.5 Gestion du journal applicatif"


class PurgeAppLogScreen(OmegaScreen):
    """Purge du journal applicatif selon 4 modes (patron #3)."""
    ...
    def compose(self) -> ComposeResult:
        ...
        with Vertical(classes="omega-form-panel"):
            yield Static("GESTION DU JOURNAL APPLICATIF", classes="omega-title")
            yield Static(
    ...
    def _validate_and_confirm(self) -> None:
        ...
        if mode == "30d":
            ...
            if not raw_count.isdigit() or int(raw_count) <= 0:
                ...
                return
            count = int(raw_count)
            removal_label = f"les {count} entrees les plus anciennes"
    ...
    def _delete_if_confirmed(self, confirmed: bool | None) -> None:
        ...
        try:
            app_logger = self._container.app_logger
        except Exception as e:
            self.app.notify(f"Logger applicatif indisponible : {e}", severity="error")
            self._pending = None
            return
        if app_logger is None:
            self.app.notify("Logger applicatif indisponible.", severity="error")
            self._pending = None
            return

        try:
            if self._pending["mode"] == "count":
                removed = app_logger.delete_oldest(self._pending["count"])
            else:
                removed = app_logger.clear(older_than=self._pending["older_than"])
        except Exception as e:
